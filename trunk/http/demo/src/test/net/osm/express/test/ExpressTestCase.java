...
import osm.http.express.ExpressServer;
...
public class ExpressTestCase extends TestCase
{
    ...
    {
        URI uri = URI.create( "link:part:osm/http/osm-http-express" );
        ...
        ExpressServer server = provider.getInstance( ExpressServer.class );
        
        //ServletContextHandler context = server.getContext( ServletContextHandler.class, "/" ), 
        //context.addServlet( uri, "/hello" );
        
        Servlet servlet = loadServlet();
        component.terminate();
    }
    
    private Servlet loadServlet() throws Exception
    {
        URI uri = getPartURI();
        Strategy strategy = Strategy.load( uri );
        return strategy.getInstance( Servlet.class );
    }
