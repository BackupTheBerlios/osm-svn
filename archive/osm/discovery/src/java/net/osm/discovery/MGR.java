    ...
    public static void main(String[] args) {
	  ...
	  try{
		...
		// create a filter and resolve against directory
	      DefaultCompositeFilter f = new DefaultCompositeFilter( );

		if( title != null ) {
		    f.add( (Filter) new DefaultContentFilter( "title", title ));
		}

		if( desc != null ) {
		    f.add( (Filter) new DefaultContentFilter( "description", desc ));
		}
		...
		if( policy > -1 ) {
		    f.add( new DefaultScalarFilter( "policy", policy ));
		}

		if( after != null ) {
		    ScalarFilter sf = new DefaultScalarFilter( "timestamp", new UtcTBase(after) );
