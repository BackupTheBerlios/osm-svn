...
import net.osm.discovery.CompositeFilter;
import net.osm.discovery.util.*;

/**
ValueFactory for net.osm.discovery.CompositeFilter 
*/

public class DefaultCompositeFilter extends CompositeFilter implements ValueFactory, FilterOperations
{
    ...
    public DefaultCompositeFilter( String name ){
	  this( name, new Filter[0] );
    }

    public DefaultCompositeFilter( String name, Filter[] filters ){
	  this.logical = true;
	  this.binary = true;
	  this.name = name;
    ...
    public java.io.Serializable read_value( InputStream is ){
	  return is.read_value( new DefaultCompositeFilter() );
    }
    ...
    public Filter newFilter( String name, Filter[] filters ) {
	  ...
	  if( name.indexOf( keyDelimiter ) > 0 ) {
		...
		return ((DefaultCompositeFilter)f).newFilter( name.substring( i+1, name.length()), filters );
	  } else {
		try{
		    return (Filter) this.lookup( name );
		} catch (Exception e) {
	          Filter f = new DefaultCompositeFilter( name, filters );
	          this.add( f );
	          return f;
		}
	  }
    ...
    public Filter newFeatureFilter( String name, String value ) {
	  ...
	  try{
		...
		v = new DefaultCompositeFilter( "value" );
		this.add( v );
        }
	  Filter f = new DefaultContentFilter( name, value) ;
	  ((DefaultCompositeFilter)v).add( (Filter) f );
	  return f;
    };
    ...
    public Filter newKeyFilter( String path ) {
	  ...
	  Filter f = ((DefaultCompositeFilter)v).newFilter( path );
	  return f;
    };
