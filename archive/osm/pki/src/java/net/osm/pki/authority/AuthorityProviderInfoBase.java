...
implements ValueFactory
{
    ...
    */
    ...
    public String toString()
    {
        ...
        buffer.append( "\tclass: " + getClass().getName() + "\n" );
        buffer.append( "\tstandard: " + standardDescription + "\n" );
        buffer.append( "\tstandard version: " + standardVersion + "\n" );
        buffer.append( "\tvendor: " + productVendor + "\n" );
        buffer.append( "\tproduct description: \n\n" );
        buffer.append( packageString( productDescription, 4, 50 ) + "\n\n" );
        buffer.append( "\tversion: " + productVersion + "\n" );	

        buffer.append( "\tsupported certificates:\n" );
        for( int i=0; i<supportedCertificates.length; i++ )
        {
            buffer.append( "\t\t" + supportedCertificates[i] + "\n" );
        }

        buffer.append( "\tsupported CRL:\n" );
        for( int i=0; i<supportedCRLs.length; i++ )
        {
            buffer.append( "\t\t" + supportedCRLs[i] + "\n" );
        }

        buffer.append( "\tsupported certificate request types:\n" );
        for( int i=0; i<supportedCertRequestTypes.length; i++ )
        {
            buffer.append( "\t\t" + supportedCertRequestTypes[i] + "\n" );
        }

        buffer.append( "\tsupported certificate recovation types:\n" );
        for( int i=0; i<supportedCertRevocationTypes.length; i++ )
        {
            buffer.append(  "\t\t" + supportedCertRevocationTypes[i] + "\n" );
        }

        buffer.append( "\tsupported certificate recovation types:\n" );
        for( int i=0; i<supportedKeyRecoveryTypes.length; i++ )
        {
            buffer.append( "\t\t" + supportedKeyRecoveryTypes[i] + "\n" );
        }
