...
import org.omg.CosPropertyService.PropertyHelper;
...
import net.osm.hub.user.UserDelegate;
...
public class PropertySetDefDelegate extends PropertySetDefPOA implements LogEnabled, Disposable
{
   /**
    * Event domain under which structured events are issued.
    */

    private static final String EVENT_DOMAIN = "org.omg.session";
   ...
   /**
    * Internal reference to the ORB.
    */
    private ORB orb;

   /**
    * PSS storage tpe for named properties.
    */
    private PropertySetDefStorage store;

   /**
    * Value used in conjunction with a property name to establish a 
    * unique key of "scope","property-name".
    */
    private byte[] scope;
        
   /**
    * The UserDelegate against which property modification events will be issued.
    */
    ...
    * scope declared by the byte array 'scope' with publication of change event 
    * against the supplied StructuredEventSource.
    */

    public PropertySetDefDelegate ( final ORB orb, final PropertySetDefStorage store, final byte[] scope, final UserEventSource source )
    {
        this.orb = orb;
	  this.store = store;
	  this.source = source;
   ...
   /**
    * Notification by the User to dispose of this instance.
    */ 
   ...
   /**
    * Creation of a new 'property' StructuredEvent, fired when a property 
    * value is modified.
    *
    * @param old org.omg.CosPropertyService.Property the old value
    * @param _new org.omg.CosPropertyService.Property the new value
    * @return 'property' structured event
    */
    
    public StructuredEvent newPropertyEvent( String feature, Property  old, Property _new )
    {

        Any a = orb.create_any();
        UserHelper.insert( a, source.getUserReference() );
        org.omg.CosNotification.Property source = new org.omg.CosNotification.Property( "source", a );

        Any b = orb.create_any();
        if( null != b ) PropertyHelper.insert( b, old );
        org.omg.CosNotification.Property oldProp = new org.omg.CosNotification.Property( "old", b );

        Any c = orb.create_any();
        if( null != c ) PropertyHelper.insert( c, _new );
        org.omg.CosNotification.Property newProp = new org.omg.CosNotification.Property( "new", c );

	  org.omg.CosNotification.Property timestamp = StructuredEventUtilities.timestamp( orb );
        return StructuredEventUtilities.createEvent( orb, propertyEventType, 
		new org.omg.CosNotification.Property[]{ timestamp, source, oldProp, newProp });
    }
    ...
    */
    ...
    UnsupportedProperty, UnsupportedMode, ReadOnlyProperty
    {
	  ...
	  catch( NotFound nf )
	  {
        ...
        if ( 
		tmpValue != null && tmpValue.property_mode.value() == PropertyModeType._read_only 
        ...
        this.store.put( this.scope, property_name, property_value, property_mode );
	  this.source.post( newPropertyEvent( property_name, null, new Property( property_name, property_value )));
    }
